...
CREATE  TABLE IF NOT EXISTS `bugware0`.`pci_all` (
  `pciSpcID` INT UNSIGNED NOT NULL AUTO_INCREMENT ,
  `uProgifID_FK` TINYINT UNSIGNED NOT NULL ,
  `uPcisubID_FK` INT UNSIGNED NOT NULL ,
  PRIMARY KEY (`pciSpcID`) ,
  INDEX `fk_pci_all_pci_prog_ifs1` (`uProgifID_FK` ASC) ,
  INDEX `fk_pci_all_pci_subsystems1` (`uPcisubID_FK` ASC) ,
  CONSTRAINT `fk_pci_all_pci_prog_ifs1`
    FOREIGN KEY (`uProgifID_FK` )
    REFERENCES `bugware0`.`pci_prog_ifs` (`uProgifID` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_pci_all_pci_subsystems1`
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `bugware0`.`sp_pciClassesAdd`(IN class_code CHAR(2), IN class_name VARCHAR(100))
BEGIN
...
-- -----------------------------------------------------
CREATE DEFINER=`root`@`localhost` PROCEDURE `bugware0`.`sp_pciProgifsAdd`(
...
-- -----------------------------------------------------
CREATE DEFINER=`root`@`localhost` PROCEDURE `bugware0`.`sp_pciSubclassesAdd`(
